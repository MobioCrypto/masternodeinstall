...
sudo ufw allow 9771/tcp &>/dev/null
sudo ufw logging on &>/dev/null
echo "y" | sudo ufw enable &>/dev/null
echo -e "${GREEN}Completion: 90%...${NC}"
sleep 2
sudo chmod +x /root/masternodeinstall/mobiod /root/masternodeinstall/mobio-cli
sudo mv /root/masternodeinstall/mobiod /root/masternodeinstall/mobio-cli /usr/local/bin
sleep 2
sudo mkdir /root/.mobio
echo "rpcuser=user"`shuf -i 100000-10000000 -n 1` >> /root/.mobio/mobio.conf
echo "rpcpassword=pass"`shuf -i 100000-10000000 -n 1` >> /root/.mobio/mobio.conf
echo "rpcallowip=127.0.0.1" >> /root/.mobio/mobio.conf
echo "listen=1" >> /root/.mobio/mobio.conf
echo "server=1" >> /root/.mobio/mobio.conf
echo -e "${GREEN}Completion: 95%...${NC}"
echo "daemon=1" >> /root/.mobio/mobio.conf
echo "maxconnections=250" >> /root/.mobio/mobio.conf
echo "masternode=1" >> /root/.mobio/mobio.conf
echo "masternodeaddr$IP:9771" >> /root/.mobio/mobio.conf
echo "externalip$IP:9771" >> /root/.mobio/mobio.conf
echo "masternodeprivkey=$KEY" >> /root/.mobio/mobio.conf
echo -e "${GREEN}Completion: 99%...${NC}"
cd /root
sudo mobiod &>/dev/null
echo -e "${GREEN}Completion: 100%...${NC}"
echo -e "Thank you for installing the Mobio masternode.  Please move onto the ${RED}NEXT${NC} step."
